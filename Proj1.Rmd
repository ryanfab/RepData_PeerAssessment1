...
```{r Steps_Histogram, warning=FALSE}
...
There are `r sum(is.na(Data$steps))` missing values in the data, accounting for `r round(100*(mean(is.na(Data$steps))),2)`% of the data. To deal with the missing values, I will replace all NA's in the step column with the mean for that interval over all days.
